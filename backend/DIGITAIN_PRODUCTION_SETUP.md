...
```bash
# Test live endpoint
curl "https://funbet-odds-fix.preview.emergentagent.com/api/digitain/live" | jq '.data[] | select(.sport_title | contains("Cricket"))'

# Test prematch endpoint
curl "https://funbet-odds-fix.preview.emergentagent.com/api/digitain/prematch?days_ahead=7" | jq '.data[] | select(.sport_title | contains("Cricket"))'
```
