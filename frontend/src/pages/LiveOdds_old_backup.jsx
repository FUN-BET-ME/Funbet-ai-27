...
const LiveOdds = () => {
  ...
  // Fetch all odds when "All" filter is selected or timeFilter changes
  const fetchAllOdds = useCallback(async (loadMore = false) => {
    if (!loadMore) setLoading(true);
    
    try {
      const BACKEND_URL = process.env.REACT_APP_BACKEND_URL;
      
      // If showing Recent Results, fetch historical odds instead
      if (timeFilter === 'recent-results') {
        const historicalData = await fetchHistoricalOdds();
        setAllOdds(historicalData);
        setScores([]); // Scores are already included in historical data
        setLastUpdated(new Date());
        setLoading(false);
        
        // Fetch team logos for recent results matches
        fetchTeamLogos(historicalData);
        return;
      }
      
      // If showing Live In-Play, fetch in-play odds instead
      if (timeFilter === 'inplay') {
        const inplayData = await fetchInPlayOdds();
        setAllOdds(inplayData);
        setScores([]); // Scores are already included in in-play data
        setLastUpdated(new Date());
        setLoading(false);
        
        // Fetch team logos for in-play matches
        fetchTeamLogos(inplayData);
        return;
      }
      
      // ========== OPTIMIZED: Paginated loading for fast initial render ==========
      // Load first 100 matches instantly, then paginate for more
      console.log('⚡ Fetching sports from database cache with pagination (instant load)...');
      
      // Calculate skip for pagination
      const currentSkip = loadMore ? allOdds.length : 0;
      const limit = 100;
      
      const [allSportsData, scoresData] = await Promise.all([
        // Paginated database call - returns 100 matches sorted by priority
        axios.get(`${BACKEND_URL}/api/odds/all-cached`, {
          params: {
            limit: limit,
            skip: currentSkip
          }
        }),
        loadMore ? Promise.resolve(scores) : fetchScores(), // Don't refetch scores when loading more
      ]);
      
      // Extract matches and pagination info from new response structure
      const responseData = allSportsData?.data || {};
      const newMatches = responseData.matches || responseData || []; // Support both old and new format
      const pagination = responseData.pagination || { has_more: false };
      
      // Merge with existing matches if loading more
      const uniqueMatches = loadMore ? [...allOdds, ...newMatches] : newMatches;
      
      // Store pagination state
      setHasMore(pagination.has_more);
      
      console.log(`✅ Loaded ${newMatches.length} matches (Total: ${uniqueMatches.length}, Has more: ${pagination.has_more})`);

      // Helper function to determine if match is completed (finished within last 35 hours)
      const isRecentlyCompleted = (match) => {
        const now = new Date();
        const matchTime = new Date(match.commence_time);
        const hoursSinceStart = (now - matchTime) / (1000 * 60 * 60);
        
        // Match is recently completed if:
        // 1. It's marked as completed in the data, OR
        // 2. It started 2-35 hours ago (typical match duration + buffer)
        return match.completed || (hoursSinceStart > 2 && hoursSinceStart < 35);
      };

      // Filter out live matches if showing "Upcoming" only
      const filteredByTime = timeFilter === 'live-upcoming' 
        ? uniqueMatches.filter(match => {
            const now = new Date();
            const matchTime = new Date(match.commence_time);
            // Only show upcoming matches (not started yet)
            return matchTime > now;
          })
        : uniqueMatches;

      // DAY-FIRST, THEN LEAGUE PRIORITY SORTING
      // PRIMARY: Day (Today before Tomorrow), SECONDARY: League Priority (within same day), TERTIARY: Time
      const limitedMatches = filteredByTime
        .sort((a, b) => {
          const now = new Date();
          const aTime = new Date(a.commence_time);
          const bTime = new Date(b.commence_time);
          
          // 1. LIVE GAMES FIRST (game has started but not ended - within last 3 hours)
          const aIsLive = aTime < now && (now - aTime) < (3 * 60 * 60 * 1000) && !a.completed;
          const bIsLive = bTime < now && (now - bTime) < (3 * 60 * 60 * 1000) && !b.completed;
          
          if (aIsLive && !bIsLive) return -1;
          if (!aIsLive && bIsLive) return 1;
          
          // 2. PRIMARY SORT: TIME BUCKETS (Day-based grouping)
          // Group matches by day: Today → Tomorrow → Day After → Future
          const getTimeBucket = (matchTime) => {
            const hoursUntil = (matchTime - now) / (1000 * 60 * 60);
            if (hoursUntil < 0) return 0; // Past (shouldn't appear)
            if (hoursUntil < 6) return 1;  // Starting very soon (< 6 hours)
            if (hoursUntil < 24) return 2; // Today
            if (hoursUntil < 48) return 3; // Tomorrow
            if (hoursUntil < 72) return 4; // Day after tomorrow
            return 5; // Future (3+ days away)
          };
          
          const aTimeBucket = getTimeBucket(aTime);
          const bTimeBucket = getTimeBucket(bTime);
          
          // PRIMARY: Sort by time bucket first (Today before Tomorrow)
          if (aTimeBucket !== bTimeBucket) return aTimeBucket - bTimeBucket;
          
          // 3. SECONDARY SORT: LEAGUE PRIORITY (within same day)
          // Within same day, Champions League appears before lower leagues
          const getPriority = (match) => {
            const sport = (match.sport_title || '').toLowerCase();
            // Tier 1: Champions League, Europa League, International
            if (sport.includes('champions league') || sport.includes('uefa champions')) return 1;
            if (sport.includes('europa league') || sport.includes('uefa europa')) return 2;
            if (sport.includes('world cup') || sport.includes('international')) return 3;
            // Tier 2: Top 5 European leagues
            if (sport.includes('premier league') || sport.includes('epl')) return 4;
            if (sport.includes('la liga') || sport.includes('spain')) return 5;
            if (sport.includes('serie a') || sport.includes('italy')) return 6;
            if (sport.includes('bundesliga') || sport.includes('germany')) return 7;
            if (sport.includes('ligue 1') || sport.includes('france')) return 8;
            // Tier 3: Major American/Cricket
            if (sport.includes('nba') || sport.includes('nfl')) return 9;
            if (sport.includes('ipl') || sport.includes('twenty20')) return 10;
            // Tier 4: Other leagues
            if (sport.includes('liga mx') || sport.includes('mls')) return 11;
            return 20; // Lower priority for other leagues
          };
          
          const aPriority = getPriority(a);
          const bPriority = getPriority(b);
          
          // SECONDARY: Within same day, sort by league priority
          if (aPriority !== bPriority) return aPriority - bPriority;
          
          // 4. TERTIARY SORT: Exact time (within same day + same league priority)
          return aTime - bTime;
        })
        .slice(0, 200);

      setScores(scoresData);
      setAllOdds(limitedMatches);
      setLastUpdated(new Date());
      
      // Fetch team logos for all matches
      fetchTeamLogos(limitedMatches);
    } catch (error) {
  ...
  const fetchTeamLogos = useCallback(async (matches) => {
    try {
      ...
      // OPTIMIZED: Fetch ALL logos in ONE API call instead of 200+ individual calls
      const response = await axios.get(`${BACKEND_URL}/api/teams/logos`);
      const allLogos = response.data || [];
      
      // Convert array to object for quick lookup
      const logosMap = {};
      allLogos.forEach(logoData => {
        if (logoData.team_name && logoData.logo_url) {
          logosMap[logoData.team_name] = logoData.logo_url;
        }
      });
      
      setTeamLogos(prev => ({ ...prev, ...logosMap }));
      console.log(`✅ Loaded ${allLogos.length} team logos in one API call`);
  ...
  // DATA LOADING EFFECTS - AFTER ALL FUNCTIONS ARE DEFINED
  useEffect(() => {
    let isMounted = true;
    
    const loadData = async () => {
      if (filter === 'all' && isMounted) {
        await fetchAllOdds();
  ...
  }, [filter, refreshKey, timeFilter, fetchAllOdds, fetchAIPredictions]);

  // Auto-refresh for live scores and odds
  useEffect(() => {
    if (filter === 'all') {
      const refreshInterval = timeFilter === 'inplay' ? 60000 : 120000;
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-400 text-lg mb-6">
            {timeFilter === 'inplay'
              ...
              : 'Compare real-time odds from top bookmakers around the world for upcoming matches in the next 7 days. Auto-refreshes every 2 minutes.'}
          </p>
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              <>
                {(() => {
                  // Apply filters
                  const filteredMatches = (showAllMatches ? allOdds : allOdds.slice(0, 50))
                    .filter(match => {
                      // First filter: must have odds
                      if (!match.bookmakers || match.bookmakers.length === 0) return false;
                      
                      // Sport filter logic (instant client-side filtering)
                      if (filter !== 'all') {
                        const sportKey = match.sport_key?.toLowerCase() || '';
                        const sportTitle = match.sport_title?.toLowerCase() || '';
                        
                        // Map filter to sport keys
                        const sportMap = {
                        ...
                        const sportKeywords = sportMap[filter] || [];
                        const matchesSport = sportKeywords.some(keyword => 
                          sportKey.includes(keyword) || sportTitle.includes(keyword)
                        );
                      ...
                      // Time filter logic
                      if (timeFilter === 'recent-results') {
                        // Show matches that finished in last 48 hours (2 days)
                        // These are matches that started 2-48 hours ago (typical match duration + buffer)
                        return hoursSinceStart > 2 && hoursSinceStart < 48;
                      } else {
                        // 'live-upcoming': Show live matches and upcoming matches
                        // Exclude matches that finished more than 2 hours ago
                        return hoursSinceStart < 2 || matchTime > now;
                      }
                    });
              ...
              {/* Show More/Less Button for All Sports - Calculate filtered count */}
              {(() => {
                const now = new Date();
                const filteredCount = allOdds.filter(match => {
                  if (!match.bookmakers || match.bookmakers.length === 0) return false;
                  const matchTime = new Date(match.commence_time);
                  const hoursSinceStart = (now - matchTime) / (1000 * 60 * 60);
                      ...
                      >
                        {showAllMatches ? (
                          ...
                          <>
                            ...
                            Show {filteredCount - 50} More Matches (Total: {filteredCount})
                          </>
                        )}
                      </button>
                    </div>
